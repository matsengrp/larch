...
TreeCount::Weight TreeCount::ComputeLeaf(const MADAG& dag, NodeId node_id) { return 1; }
...
std::pair<TreeCount::Weight, std::vector<size_t>> TreeCount::WithinCladeAccumOptimum(
  ...
  return {std::accumulate(inweights.begin(), inweights.end(), (TreeCount::Weight)0,
                          std::plus<TreeCount::Weight>()),
          indices};
}

TreeCount::Weight TreeCount::BetweenClades(std::vector<TreeCount::Weight> inweights) {
  return std::accumulate(inweights.begin(), inweights.end(), (TreeCount::Weight)1,
                         std::multiplies<TreeCount::Weight>());
}

TreeCount::Weight TreeCount::AboveNode(TreeCount::Weight edgeweight,
