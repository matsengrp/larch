...
std::pair<TreeCount::Weight, std::vector<size_t>> TreeCount::WithinCladeAccumOptimum(std::vector<TreeCount::Weight> inweights) {
    ...
    return {std::accumulate(inweights.begin(), inweights.end(), (TreeCount::Weight) 0, std::plus<TreeCount::Weight>()), indices};
}

TreeCount::Weight TreeCount::BetweenClades(std::vector<TreeCount::Weight> inweights) {
    return std::accumulate(inweights.begin(), inweights.end(), (TreeCount::Weight) 1, std::multiplies<TreeCount::Weight>());
}
