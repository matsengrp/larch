...
FeatureMutableView<ReferenceSequence, CRTP, Tag>::BuildCladeUnionMap() const {
  auto dag = static_cast<const CRTP&>(*this);
  using CladeUnion = std::set<NodeId>;
  using CladeUnionMap = std::map<CladeUnion, std::set<NodeId>>;

  CladeUnionMap clade_union_map;
  dag.GetRoot().CalculateLeafsBelow();
  for (auto node : dag.GetNodes()) {
    CladeUnion full_leafset;
