...
[[maybe_unused]] static void test_compact_genome(std::string_view pb_path,
  ...
  assert_equal(json_dag.reference_sequence, pb_dag.reference_sequence,
               "Reference sequence");
  assert_equal(pb_dag.ComputeCompactGenomesDAG(json_dag.reference_sequence),
               json_dag.compact_genomes, "Compact genomes");
}
...
[[maybe_unused]] static void test_edge_mutations(std::string_view path) {
  ...
  std::vector<std::reference_wrapper<MADAG>> tree_refs;
