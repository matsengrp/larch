...
#include "../usher-build/version.hpp"
...
void log_flush(int) { fflush(movalbe_src_log); }
...
void print_file_info(std::string info_msg, std::string error_msg,
                     const std::string& filename) {
  struct stat stat_buf;
  errno = 0;
  if (stat(filename.c_str(), &stat_buf) == 0) {
    fprintf(stderr, "%s %s last modified: %s", info_msg.c_str(), filename.c_str(),
...
int main(int argc, char** argv) {
  ...
  desc.add_options()("vcf,v",
                     po::value<std::string>(&input_vcf_path)->default_value(""),
                     "Input VCF file (in uncompressed or gzip-compressed .gz format) ")(
      "tree,t", po::value<std::string>(&input_nh_path)->default_value(""),
      "Input tree file")("threads,T",
                         po::value<uint32_t>(&num_threads)->default_value(num_cores),
                         num_threads_message.c_str())(
      "load-mutation-annotated-tree,i",
      po::value<std::string>(&input_pb_path)->default_value(""),
      ...
      "[REQUIRED]")("radius,r", po::value<int32_t>(&radius)->default_value(-1),
                    "Radius in which to restrict the SPR moves.")(
      "profitable-src-log,S",
      po::value<std::string>(&profitable_src_log)->default_value("/dev/null"),
      "The file to log from which node a profitable move can be found.")(
      "ambi-protobuf,a",
      po::value<std::string>(&input_complete_pb_path)->default_value(""),
      "Continue from intermediate protobuf")(
      "minutes-between-save,s",
      po::value<unsigned int>(&minutes_between_save)->default_value(0),
      "Minutes between saving intermediate protobuf")(
      "min-improvement,m", po::value<float>(&min_improvement)->default_value(0.0005),
      "Minimum improvement in the parsimony score as a fraction of the previous score "
      "in ordder to perform another iteration.")(
      "drift_iteration,d", po::value<int>(&drift_iterations)->default_value(0),
      "Iterations permiting equally parsimonious moves after parsimony score no longer "
      "improves")("do-not-write-intermediate-files,n",
                  "Do not write intermediate files.")(
      "max-iterations,N", po::value<int>(&max_round)->default_value(1000),
      "Maximum number of optimization iterations to perform.")(
      "max-hours,M", po::value(&max_optimize_hours)->default_value(0),
      "Maximium number of hours to run")(
      "transposed-vcf-path,V", po::value(&transposed_vcf_path)->default_value(""),
      "Auxiliary transposed VCF for ambiguous bases, used in combination with usher "
      "protobuf (-i)")("version", "Print version number")(
      "node_proportion,z", po::value(&search_proportion)->default_value(2),
      "the proportion of nodes to search")("node_sel,y", po::value(&rand_sel_seed),
                                           "Random seed for selecting nodes to search")(
      "drift_nwk_file,b", po::value(&intermediate_nwk_out)->default_value(""),
      "Newick filename stem for drifting")(
  ...
  if (max_optimize_hours) {
    search_end_time = std::chrono::steady_clock::now() +
                      std::chrono::hours(max_optimize_hours) - std::chrono::minutes(30);
  ...
  if (this_rank == 0) {
    ...
    try {
      auto output_path_dir_name =
          boost::filesystem::system_complete(output_path).parent_path();
    ...
    auto fd =
        open(output_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (fd == -1) {
    ...
    if (input_complete_pb_path != "") {
      print_file_info("Continue from", "continuation protobut,-a",
                      input_complete_pb_path);
    } else if (input_pb_path != "") {
      if (input_vcf_path == "") {
        print_file_info("Load both starting tree and sample variant from",
                        "input protobut,-i", input_pb_path);
      } else {
        print_file_info("Extract starting tree from", "input protobut,-i",
                        input_pb_path);
        print_file_info("Load sample variant from", "sample vcf,-v", input_vcf_path);
      }
    } else if (input_nh_path != "" && input_vcf_path != "") {
      print_file_info("Load starting tree from", "starting tree file,-t",
                      input_nh_path);
      print_file_info("Load sample variant from", "sample vcf,-v", input_vcf_path);
    } else {
      fputs(
          "Input file not completely specified. Please either \n"
          "1. Specify an intermediate protobuf from last run with -a to continue "
          "optimization, or\n"
          "2. Specify a usher-compatible protobuf with -i, and both starting tree and "
          "sample variants will be extracted from it, or\n"
          "3. Specify a usher-compatible protobuf with -i for starting tree, and a VCF "
          "with -v for sample variants, or\n"
          "4. Specify the starting tree in newick format with -t, and a VCF with -v "
          "for sample variants.\n",
          stderr);
      std::cerr << desc << std::endl;
      exit(EXIT_FAILURE);
    ...
    if (no_write_intermediate) {
      fprintf(stderr,
              "Will not write intermediate file. WARNNING: It will not be possible to "
              "continue optimization if this program is killed in the middle.\n");
    } else {
    ...
    if (profitable_src_log != "/dev/null") {
      log_moves = true;
      fprintf(stderr,
              "Will write list of source nodes where a profitable move can be found to "
              "%s. \n",
    ...
    fprintf(stderr,
            "Run kill -s SIGUSR2 %d to apply all the move found immediately, then "
            "output and exit.\n",
            pid);
    ...
    fprintf(movalbe_src_log,
            "source\tdestination\titeration\tscore.change\tdistance\tsubtree.size\n");
    bool isfirst = true;
    ...
    while (stalled < drift_iterations) {
      ...
      if (search_proportion < 1) {
        ...
        std::sample(nodes_to_search.begin(), nodes_to_search.end(),
                    ...
                    size_t(std::round(nodes_to_search.size() * search_proportion)),
                    rng);
      ...
      while (!nodes_to_search.empty()) {
        ...
        fprintf(stderr,
                "parsimony score after optimizing: %zu,with radius %d, second from "
                "start %ld \n\n",
