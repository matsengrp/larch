...
static void clean_up_leaf(std::vector<MAT::Node*>& dfs) {
  tbb::parallel_for(
      ...
      [&dfs](tbb::blocked_range<size_t> range) {
...
void leader_thread_optimization(MAT::Tree& tree,
                                std::vector<mutated_t>& position_wise_out,
                                std::atomic_size_t& curr_idx, int& optimization_radius,
                                size_t start_idx, FILE* ignored_file,
                                float desired_optimization_msec, bool is_last) {
  int last_parsimony_score = INT_MAX;
  std::default_random_engine g;
  auto optimiation_start = std::chrono::steady_clock::now();
  auto optimization_end =
      optimiation_start + std::chrono::milliseconds((long)desired_optimization_msec);
  bool timeout = false;
  if (is_last) {
  ...
  do {
    ...
    find_moved_node_neighbors(optimization_radius, start_idx, tree, curr_idx.load(),
                              node_to_search_idx);
    ...
    while (!node_to_search_idx.empty()) {
      ...
      optimize_tree_main_thread(node_to_search_idx, tree, optimization_radius,
                                ignored_file, false, 1, deferred_nodes_out, distributed,
                                optimization_end, true, true, true);
  ...
  if (finish_time > optimization_end) {
    ...
    float actual_msec = std::chrono::duration_cast<std::chrono::milliseconds>(
                            finish_time - optimiation_start)
                            .count();
...
static int leader_thread(int batch_size_per_process, Leader_Thread_Options& options) {
  ...
  if (options.tree_in != "") {
    ...
    tree = MAT::load_mutation_annotated_tree(options.protobuf_in);
  }
  ...
  Sample_Input(options.vcf_filename.c_str(), samples_to_place, tree, position_wise_out,
               options.override_mutations, samples);
  ...
  fprintf(stderr, "Sample start idx %d, end index %d\n", sample_start_idx,
          sample_end_idx);
  if (options.tree_in == "" && (!options.no_add) &&
  ...
  if (options.tree_in != "") {
    for (auto& pos : position_wise_out_dup) {
      pos.erase(
          std::remove_if(pos.begin(), pos.end(),
                         [sample_start_idx](const std::pair<long, nuc_one_hot>& in) {
                           return in.first < sample_start_idx;
                         }),
          pos.end());
    }
  ...
  if (options.no_add) {
    ...
    place_sample_leader(samples_to_place, tree, 100, curr_idx, INT_MAX, true,
                        placement_stats_file, INT_MAX, INT_MAX, low_confidence_samples,
                        samples_clade, sample_start_idx, nullptr, true);
    print_annotation(tree, options.out_options, samples_clade, sample_start_idx,
                     sample_end_idx, tree.get_num_annotations());
    MPI_Finalize();
    return 0;
  }
  ...
  if (options.keep_n_tree > 1) {
    ...
    place_sample_multiple_tree(samples_to_place, trees, placement_stats_file,
                               options.keep_n_tree);
    for (int t_idx = 0; t_idx < trees.size(); t_idx++) {
      std::vector<Clade_info> assigned_clades;
      std::vector<std::string> low_confidence_samples;
      final_output(trees[t_idx], options.out_options, t_idx, assigned_clades,
  ...
  while (true) {
    ...
    place_sample_leader(samples_to_place, tree, batch_size_per_process, curr_idx,
                        options.parsimony_threshold, false, placement_stats_file,
                        options.max_parsimony, options.max_uncertainty,
                        low_confidence_samples, samples_clade, sample_start_idx,
                        idx_map_ptr);
    bool is_last = false;
    tree.check_leaves();
    ...
    if (options.initial_optimization_radius > 0) {
      leader_thread_optimization(tree, position_wise_out, curr_idx, optimization_radius,
                                 sample_start_idx, ignored_file,
                                 is_last ? 60000 * options.last_optimization_minutes
                                         : options.desired_optimization_msec,
                                 is_last);
      tree.check_leaves();
    }
    if (curr_idx < samples_to_place.size()) {
...
int main(int argc, char** argv) {
  ...
  desc.add_options()(
      ...
      "Input VCF file (in uncompressed or gzip-compressed .gz format) [REQUIRED]")(
      "tree,t", po::value<std::string>(&options.tree_in)->default_value(""),
      "Input tree file")(
      "outdir,d",
      ...
      po::value<std::string>(&options.protobuf_in)->default_value(""),
      "Load mutation-annotated tree object")(
      "save-mutation-annotated-tree,o",
      po::value<std::string>(&options.out_options.dout_filename)->default_value(""),
      ...
      "Sort new samples based on computed parsimony score and then number of optimal "
      "placements before the actual placement [EXPERIMENTAL].")(
      "sort-before-placement-2,S",
      po::bool_switch(&options.sort_before_placement_2)->default_value(false),
      "Sort new samples based on the number of optimal placements and then the "
      ...
      "Reverse the sorting order of sorting options (sort-before-placement-1 or "
      "sort-before-placement-2) [EXPERIMENTAL]")(
      "collapse-tree,c", po::bool_switch(&options.collapse_tree)->default_value(false),
      "Collapse internal nodes of the input tree with no mutations and condense "
      "identical sequences in polytomies into a single node and the save the tree to "
      "file condensed-tree.nh in outdir")(
      "collapse-output-tree,C", po::bool_switch(&ignored_options),
      "Collapse internal nodes of the output tree with no mutations before the saving "
      "the tree to file final-tree.nh in outdir")(
      "max-uncertainty-per-sample,e",
      po::value(&options.max_uncertainty)->default_value(1e6),
      "Maximum number of equally parsimonious placements allowed per sample beyond "
      "which the sample is ignored")(
      "max-parsimony-per-sample,E",
      po::value(&options.max_parsimony)->default_value(1e6),
      "Maximum parsimony score of the most parsimonious placement(s) allowed per "
      "sample beyond which the sample is ignored")(
      ...
      po::value<size_t>(&options.out_options.print_subtrees_size)->default_value(0),
      "Write minimum set of subtrees covering the newly added samples of size equal to "
      "this value")(
      "write-single-subtree,K",
      po::value<size_t>(&options.out_options.print_subtrees_single)->default_value(0),
      "Similar to write-subtrees-size but produces a single subtree with all newly "
      "added samples along with random samples up to the value specified by this "
      "argument")(
      "multiple-placements,M", po::value(&options.keep_n_tree)->default_value(1),
      "Create a new tree up to this limit for each possibility of parsimony-optimal "
      "placement")("retain-input-branch-lengths,l",
      ...
      "In clades.txt, write a histogram of annotated clades and counts across all "
      "equally parsimonious placements")(
      "threads,T", po::value<uint32_t>(&num_threads)->default_value(num_cores),
      num_threads_message.c_str())("version", "Print version number")(
      "help,h", "Print help messages")(
      "reduce-back-mutation,B",
      po::bool_switch(&options.out_options.redo_FS_Min_Back_Mutations)
          ->default_value(false),
      "Reassign states of internal nodes to reduce back mutation count.")(
      "version", "Print version number")("help,h", "Print help messages")(
      "optimization_radius",
      po::value(&options.initial_optimization_radius)->default_value(4),
      "The search radius for optimization when parsimony score increase exceeds the "
      "threshold"
      ...
      "Set to 0 to disable optimiation"
      "Only newly placed samples and nodes within this radius will be searched")(
      "last_optimization_minutes",
      po::value(&options.last_optimization_minutes)->default_value(120),
      ...
      "parsimony_threshold",
      po::value(&options.parsimony_threshold)->default_value(100000),
      "Optimize after the parsimony score increase by this amount")(
      "first_n_samples", po::value(&options.first_n_samples)->default_value(SIZE_MAX),
      "[TESTING ONLY] Only place first n samples")
  ...
  if (this_rank == 0) {
    if (options.keep_n_tree > 1 && process_count > 1) {
      fprintf(stderr,
              "Multi-host parallelization of multiple placement is not supported\n");
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
...
bool sort_samples(const Leader_Thread_Options& options,
                  std::vector<Sample_Muts>& samples_to_place, MAT::Tree& tree,
                  size_t sample_start_idx) {
  bool reordered = false;
  if (options.sort_by_ambiguous_bases) {
    fprintf(stderr,
            "Sorting missing samples based on the number of ambiguous bases \n");
  ...
  if (options.collapse_tree) {
    ...
    fprintf(condensed_tree_file, "%s\n",
            tree.get_newick_string(true, true,
  ...
  if (options.print_parsimony_scores) {
    ...
    if ((options.sort_before_placement_1 || options.sort_before_placement_2) &&
      ...
      // timer.Start();
      fprintf(stderr,
              "Computing parsimony scores and number of "
      ...
      std::atomic_size_t curr_idx(0);
      place_sample_leader(samples_to_place, tree, 100, curr_idx, INT_MAX, true, nullptr,
                          options.max_parsimony, options.max_uncertainty,
                          low_confidence_samples, samples_clade, sample_start_idx,
                          nullptr);
      fputc('\n', stderr);
