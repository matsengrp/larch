...
[[noreturn]] static void Usage() {
  ...
  const std::vector<std::pair<std::string, std::string>> flag_desc_pairs = {
      ...
      {"--ignore-root_edge_mutations", "Ignore root edge mutations when computing parsimony\n"},
...
int main(int argc, char** argv) {  // NOLINT(bugprone-exception-escape)
  ...
  auto logger = [&input_dag, &merge, &logfile, &log_timer, &intermediate_dag_path,
    ...
    SubtreeWeight<UAFreeParsimonyScore, MergeDAG> min_ua_free_parsimony_scorer{
        merge.GetResult()};
    auto min_ua_free_parsimony_score = min_ua_free_parsimony_scorer.ComputeWeightBelow(root_node, {});
    auto min_ua_free_parsimony_count = min_ua_free_parsimony_scorer.MinWeightCount(root_node, {});
    // Min Sum RF Distance
  ...
  for (size_t i = 0; i < iter_count; ++i) {
    ...
    SubtreeWeight<UAFreeParsimonyScore, MergeDAG> weight_ua{merge.GetResult(), main_rng.GenerateSeed()}; 
    SubtreeWeight<BinaryParsimonyScore, MergeDAG> weight_bin{merge.GetResult(), main_rng.GenerateSeed()}; 
    ...
    auto subtree_node = [&]() -> std::optional<NodeId> {
      if (subtrees) {
        ...
        auto node_filter = [&](NodeId start_node, auto& node_filter_ref) -> void {
          auto node_instance = use_ua_free_parsimony ? weight_ua.GetDAG().Get(start_node) : weight_bin.GetDAG().Get(start_node);
        ...
        if (use_ua_free_parsimony) {
          node_filter(weight_ua.GetDAG().GetRoot().GetId(), node_filter);
        } else {
        ...
        if (not options.empty()) {
          ...
          std::cout << "Chose node " << chosen_node.value
                    ...
                    << std::flush;
          return use_ua_free_parsimony ? weight_ua.GetDAG().Get(chosen_node) : weight_bin.GetDAG().Get(chosen_node);
        } else {
    ...
    auto sample_tree = [&merge, &sample_method, &weight_bin, &weight_ua, &subtree_node, &main_rng, &use_ua_free_parsimony]() {
      if (sample_method == SampleMethod::Random) {
