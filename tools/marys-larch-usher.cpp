...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    std::cout << "dag 1(original dag): ";
    for (auto node: input_dag.GetDAG().GetNodes()) {
        if (node.GetSampleId()) {
            std::cout << node.GetSampleId().value() << ", ";
        }
    }
    std::cout << "\n\ndag 2(sample from merge): ";
    for (auto node: sample.GetDAG().GetNodes()) {
        if (node.IsLeaf() and node.GetSampleId()) {
            std::cout << node.GetSampleId().value() << ", ";
        }
    }
    std::cout << "\n\n";
    ...
    MADAG sample3 = LoadTreeFromProtobuf("optimized_tree.pb");
    sample3.GetReferenceSequence() = input_dag.GetReferenceSequence();
    sample3.GetCompactGenomes() = sample3.ComputeCompactGenomes(input_dag.GetReferenceSequence());
    std::cout << " got sample tree and computed its compact genome.\n" << std::flush;
