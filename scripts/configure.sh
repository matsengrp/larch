...
if [[ "$LARCH_USE_NETAM" == "ON" || "$LARCH_USE_NETAM" == "yes" ]]; then
  if [[ "$LARCH_LIBTORCH_PATH" == "auto" ]]; then
    if [[ -n "${CONDA_PREFIX:-}" ]]; then
      ...
      TORCH_CMAKE_DIR="$(find "$CONDA_PREFIX" -name "TorchConfig.cmake" 2>/dev/null | head -1 | xargs dirname)"
      ...
      CMAKE_ARGS+=("-DCMAKE_PREFIX_PATH=$CONDA_PREFIX;$TORCH_PREFIX")
    ...
    CMAKE_ARGS+=("-DCMAKE_PREFIX_PATH=$LIBTORCH_ABS")
...
if [[ "$LARCH_PROTOBUF_PATH" == "auto" ]]; then
  ...
  if [[ -n "${CONDA_PREFIX:-}" && -d "$CONDA_PREFIX/include/google/protobuf" ]]; then
    CMAKE_ARGS+=("-DProtobuf_INCLUDE_DIR=$CONDA_PREFIX/include")
  ...
  # Find the protobuf shared library (platform-aware)
  PROTOBUF_LIB="$(find "$PROTOBUF_ABS/lib" -name 'libprotobuf.so' -o -name 'libprotobuf.dylib' 2>/dev/null | head -1)"
  if [[ -z "$PROTOBUF_LIB" ]]; then
    echo "Error: libprotobuf shared library not found in $PROTOBUF_ABS/lib/" >&2
    exit 1
  fi
  CMAKE_ARGS+=("-DProtobuf_INCLUDE_DIR=$PROTOBUF_ABS/include")
  CMAKE_ARGS+=("-DProtobuf_LIBRARY=$PROTOBUF_LIB")
  CMAKE_ARGS+=("-DProtobuf_PROTOC_EXECUTABLE=$PROTOBUF_ABS/bin/protoc")
...
# Split LARCH_CMAKE_EXTRA into array elements (word-splitting is intentional here)
# shellcheck disable=SC2206
[[ -n "${LARCH_CMAKE_EXTRA:-}" ]] && CMAKE_ARGS+=($LARCH_CMAKE_EXTRA)
