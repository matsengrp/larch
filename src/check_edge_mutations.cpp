...
void check_edge_mutations_helper(const Node dag_node,const all_mutated_t& all_mutated,const std::string_view ref_seq, const std::vector<EdgeMutations>& edge_mutations ){ 
    for (auto child : dag_node.GetChildren()) {
        ...
        for (const auto& edge_mut : edge_mutations[edge_id.value]) {
            ...
            if(ins_result.second){
                ...
                if(expected!=actual_par){
                    std::cout<<"On edge "<<edge_id.value<<" from "<<dag_node.GetId().value<<" to "<<child.GetChildId().value<<" at position "<< edge_mut.first.value<<" expected "<<
                        expected<<" from mutation on edge "<<ins_result.first->second.edge_id.value<< "from "
                        <<ins_result.first->second.par_node_id.value<< " to "<<ins_result.first->second.child_node_id.value <<" but got "<<actual_par<<"\n";
                }
        ...
        check_edge_mutations_helper(child.GetChild(), this_mutated, ref_seq, edge_mutations);
...
void check_edge_mutations(const MADAG& madag){
    ...
    all_mutated_t init;
    check_edge_mutations_helper(madag.GetDAG().GetRoot(), init, madag.GetReferenceSequence(), madag.GetEdgeMutations());
